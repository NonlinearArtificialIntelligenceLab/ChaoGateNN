...
DUFFING_SCRIPT="./scripts/duffing_chaogate.py"
...
for gate in "${gates[@]}"; do
    ...
    for seed in $(seq 0 $((NUM_SEEDS - 1))); do
        for beta in $(seq 0 0.5 5); do
            out_path="${OUTPUT_BASE_DIR}/duffing/${gate}/beta${beta}_seed${seed}/"
            echo "Running duffing_chaogate.py for gate $gate with seed $seed and beta=$beta"
