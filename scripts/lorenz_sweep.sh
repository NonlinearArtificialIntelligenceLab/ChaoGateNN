...
LORENZ_SCRIPT="./scripts/lorenz_chaogate.py"
...
for gate in "${gates[@]}"; do
    ...
    for seed in $(seq 0 $((NUM_SEEDS - 1))); do
        for rho in $(seq 14 1 28); do
            out_path="${OUTPUT_BASE_DIR}/lorenz/${gate}/rho${rho}_seed${seed}/"
            echo "Running lorenz_chaogate.py for gate $gate with seed $seed and rho=$rho"
